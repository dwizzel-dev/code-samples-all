...
import chat.CieTabManager;
import control.CiePanel;

dynamic class chat.CieContentManager{
	...
	private  function CieContentManager(Void){
			...
			if(currNode.nodeName == 'P'){
				...
				if(contentTree[pname] == undefined){
					...
					if(currNode.attributes.bgcolor != undefined  && currNode.attributes.bgcolor != ''){	
						//DWIZZEL START
						//we will replace text __BGCOLOR_X__ with the color in the style.xml: CieStyle.__content.__bgcolor_X
						if(currNode.attributes.bgcolor == '__BGCOLOR_0__'){
							contentTree[pname]['__class'].setBgColor(CieStyle.__content.__bgcolor_0);	
						}else if(currNode.attributes.bgcolor == '__BGCOLOR_1__'){
							contentTree[pname]['__class'].setBgColor(CieStyle.__content.__bgcolor_1);
						}else{
	...
	public  function changeNodeValue(xmlNode:XMLNode, arrPanel:Array):Void{
