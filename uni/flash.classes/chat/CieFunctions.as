...
dynamic class chat.CieFunctions{
	...
	private var __intervalShakeHand:Number;
	...
	private function CieFunctions(Void){
		...
		this.__isReadyMaxLoopCounter = 14;
		};
	...
	public function setTitleForm(strTitle:String):Void{
		if(strTitle == undefined || strTitle == ''){
			mdm.Forms[BC.__system.__formName].title = 'CHAT (not connected)';
		}else{
			mdm.Forms[BC.__system.__formName].title = 'CHAT (' + strTitle + ')';
			}
		};	
	...
	/***has disconnected his camera*********************************************************************************************************************************/	
	
	public function cameraIsOff(Void):Void{
		//tell the other that we have connected our camera
		cLocalConn.callRemoteSocket(BC.__user.__remoteconn, ['CAM_OFF', BC.__user.__pseudo]);
		};	
	...
	public function updateAdminMessage(str:String):Void{
		//update text
		this.__chatOUT.txtInfos.htmlText += '<font color="#ff0000"><b>ADMINISTRATOR:</b> ' + str + '</font>\n\n' ; 
	...
	public function localSocketCommand(param:Array):Void{
		...
		if(param[1][0] == 'MSG'){
			...
			//c'est une commande update
			//Debug('CAM_ON');
			//l'autre a deconnecte sa camera alors on avertit
			secFms.otherCameraIsOff();	
			...
			if(!this.__otherUserIsReady){
				...
				setLoading(false);
				//title
				this.setTitleForm(BC.__user.__pseudo + '<->' + BC.__user.__localpseudo);
