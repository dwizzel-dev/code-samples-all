...
dynamic class manager.CieSocketManager{
	...
	public function socketCommand(strCommand:String, strData:String):Void{
		...
		if(strCommand == 'USERINFOS'){ //USERINFOS:1737771,uniadmin,1,2,1,11111
			//info sur l'usager connecte
			var arrData = strData.split(',');
			BC.__user.__nopub = arrData[0];
			BC.__user.__pseudo = arrData[1];
			BC.__user.__state = Number(arrData[2]);
			BC.__user.__membership = Number(arrData[3]);
			BC.__user.__admin = Number(arrData[4]);
			//les droits admin
			BC.__user.__rights.__kill = Number(arrData[5].substr(0,1));
			BC.__user.__rights.__delete = Number(arrData[5].substr(1,1));
			BC.__user.__rights.__create = Number(arrData[5].substr(2,1));
			BC.__user.__rights.__promote = Number(arrData[5].substr(3,1));
			BC.__user.__rights.__watch = Number(arrData[5].substr(4,1));
			//trace
			if(BC.__user.__debug){
				...
				Debug('[USER] MEMBERSHIP: ' + BC.__user.__membership);
		...
		}else if(strCommand == 'ROOMPRIVATECREATED'){	//la room d'un membre privilege est bien cree
			cFunc.confirmationCreationRoomPrivate(strData.split(","));			
