...
dynamic class irc.CieFunctions{
	...
	public function openIrc(Void):Void{
		...
		//si c'est un lien direct a partir d'un url ouvrir la salle demande par defaut
		if(__gArgs.__gotoroom != undefined && __gArgs.__gotoroom != ''){
			//on ouvre la room
			if(this.__arrRoomsDB[this.__roomPrefix + __gArgs.__gotoroom] != undefined){
				Debug('GOTO_ROOM: ' + __gArgs.__gotoroom + ' IN DB');
				cFunc.createPublicRoom(this.__roomPrefix + __gArgs.__gotoroom);
			}else{
				Debug('GOTO_ROOM: ' + __gArgs.__gotoroom + ' NOT IN DB');
				}
			}
		};
		...
		*/
		if(bPublic){ //pour les rooms
			if(this.__arrRooms[this.__roomPrefix + arrInfos[0]] != undefined){
				...
				OK alors on va faire une requete pour aller chercher les peudose que nousn'avons pas encore
	...
	public function confirmationCreationRoomPrivate(arrInfos:Array):Void{
		...
		if(arrInfos[1] == 'OK'){
			...
			//on ouvre la room qui vient d'etre cree si elle est dans la DB des rooms
			if(this.__arrRoomsDB[this.__roomPrefix + arrInfos[0]] != undefined){
				Debug('room ' + arrInfos[0] + ' is in the DB');
				cFunc.createPublicRoom(this.__roomPrefix + arrInfos[0]);
			}else{
				Debug('room ' + arrInfos[0] + ' is NOT in the DB');
				}
		}else if(arrInfos[1] == 'ERR_EXIST'){	
			new CieTextMessages('MB_OK', gLang[183] + '<b>' + arrInfos[0] + '</b>' + gLang[184], gLang[109]);	
			}
		};
	...
	public function addRoomLanguage(roomName:String, roomSlogan:String):Void{
		var arrLanguages = [['fr_CA', gLang[186]],['en_US', gLang[187]],['de_DE', gLang[188]],['ru_RU', gLang[189]],['es_ES', gLang[190]]];
		this.__cMessageBox = new CieOptionMessages(gLang[185], arrLanguages, gLang[154]);
		this.__cMessageBox.setSelectionValue(0);
		this.__cMessageBox.setCallBackFunction(this.cbAddRoomLanguage, {__class: this, __roomName:roomName, __roomSlogan:roomSlogan, __arrLanguages:arrLanguages});
		};
	...
	public function cbAddRoomLanguage(cbObject:Object):Void{
		if(cbObject.__ok == true){
			/*
			Debug("cbAddRoomLanguage:");
			Debug("arrLanguages: " + cbObject.__arrLanguages);
			Debug("__cMessageBox: " + cbObject.__class.__cMessageBox);
			Debug("__choices: " + cbObject.__class.__cMessageBox.getSelectedChoice());
			*/
			var roomLanguage = cbObject.__arrLanguages[cbObject.__class.__cMessageBox.getSelectedChoice()][0];
			if(roomLanguage == '' || roomLanguage == undefined){
				roomLanguage = 'fr_CA';
				}
			cbObject.__class.addRoomOrientation(cbObject.__roomName, cbObject.__roomSlogan, roomLanguage);
			}
		//cbObject.__class.__cMessageBox = null;	
		};
	...
	//set a room language
	public function addRoomOrientation(roomName:String, roomSlogan:String, roomLanguage:String):Void{
		var arrOrientation = [['1', gLang[191]],['2', gLang[192]],['3', gLang[193]]];
		//this.__cMessageBox = null;
		this.__cMessageBox = new CieOptionMessages(gLang[194], arrOrientation, gLang[195]);
		this.__cMessageBox.setSelectionValue(0);
		this.__cMessageBox.setCallBackFunction(this.cbAddRoomOrientation, {__class: this, __roomName:roomName, __roomSlogan:roomSlogan, __roomLanguage:roomLanguage, __arrOrientation:arrOrientation});
		};	
		
	//callback when user make a choice of selection	
	public function cbAddRoomOrientation(cbObject:Object):Void{
		if(cbObject.__ok == true){
			var roomOrientation = cbObject.__arrOrientation[cbObject.__class.__cMessageBox.getSelectedChoice()][0];
			if(roomOrientation == '' || roomOrientation == undefined){
				roomOrientation = '1'; //default
				}
			//send to server
			cbObject.__class.sendNewRoomToServer([cbObject.__roomName, cbObject.__roomSlogan, cbObject.__roomLanguage, roomOrientation]);
			}
		//cbObject.__class.__cMessageBox = null;	
		};	
	...
	public function sendNewRoomToServer(arrInfos:Array):Void{
		...
		*/
		...
		cSockManager.socketSend("CREATEROOM:" + arrInfos[0] + ',' + 't3.jpg' + ',' + escape(arrInfos[1]) + ',' + arrInfos[2] + ',' + arrInfos[3]);
		};
