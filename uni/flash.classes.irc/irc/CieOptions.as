...
dynamic class irc.CieOptions{
	...
	public function changeWelcomeText(Void):Void{
		...
		//create a room for member privilege only
		//if(BC.__user.__membership == 0 || BC.__user.__membership == 1 && BC.__user.__admin != 1){
		if(BC.__user.__membership == 0 || BC.__user.__membership == 1){
			//create new room with pseudo as default name
			strText += gLang[196] + '<a href="asfunction:cFunc.createDirectRoomPrivate"><u>' + gLang[197] + '</u></a>.';
			strText += '<br><br>';
			}
		//si admin
	...
	private function drawRoomList(Void):Void{
		...
		for(var i=0; i<arrRoomSort.length; i++){
			...
			this.__arrRoomListMovie[o].mvPlusBg.gotoAndStop('_0');
