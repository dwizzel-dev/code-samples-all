...
class Links {
	...
	public function getLinksListing(){
		$query = 'SELECT '.DB_PREFIX.'links.has_moved AS "has_moved", '.DB_PREFIX.'links.access AS "access", '.DB_PREFIX.'links.hits AS "hits", '.DB_PREFIX.'links.id AS "id", '.DB_PREFIX.'links.keyindex AS "keyindex", '.DB_PREFIX.'links.name AS "name", '.DB_PREFIX.'links.language_id AS "language_id", '.DB_PREFIX.'links.path AS "path", '.DB_PREFIX.'links.status AS "status", '.DB_PREFIX.'languages.prefix AS "language", '.DB_PREFIX.'links.extern AS "extern" FROM '.DB_PREFIX.'links LEFT JOIN '.DB_PREFIX.'languages ON '.DB_PREFIX.'languages.id = '.DB_PREFIX.'links.language_id ORDER BY '.DB_PREFIX.'links.status DESC,'.DB_PREFIX.'links.language_id ASC, '.DB_PREFIX.'links.path ASC;';
	...
	public function getLinksForDropDown(){
		$query = 'SELECT '.DB_PREFIX.'links.access AS "access", '.DB_PREFIX.'links.hits AS "hits", '.DB_PREFIX.'links.id AS "id", '.DB_PREFIX.'links.keyindex AS "keyindex", '.DB_PREFIX.'links.name AS "name", '.DB_PREFIX.'links.language_id AS "language_id", '.DB_PREFIX.'links.path AS "path", '.DB_PREFIX.'links.extern AS "extern", '.DB_PREFIX.'links.status AS "status", '.DB_PREFIX.'languages.prefix AS "language" FROM '.DB_PREFIX.'links LEFT JOIN '.DB_PREFIX.'languages ON '.DB_PREFIX.'languages.id = '.DB_PREFIX.'links.language_id WHERE '.DB_PREFIX.'links.has_moved = "0" ORDER BY '.DB_PREFIX.'links.language_id, '.DB_PREFIX.'links.path;';
		$rs = $this->reg->get('db')->query($query);
		if($rs->num_rows){
			return $rs->rows;	
			}
	...
	public function setLinksInfos($arr){
		...
		if(isset($arr) && is_array($arr) && count($arr)){
			...
			if(count($this->arrFormErrors) != 0){
				...
				$query = 'UPDATE '.DB_PREFIX.'links SET status = "'.$arrValues['status'].'", extern = "'.$arrValues['extern'].'", name = "'.$arrValues['name'].'", keyindex = "'.$arrValues['keyindex'].'", path = "'.$arrValues['path'].'", language_id = "'.$arrValues['language_id'].'", date_modified = NOW() WHERE '.DB_PREFIX.'links.id = "'.$arrValues['id'].'";';
	...
	public function newLinksInfos($arr){
		...
		if(isset($arr) && is_array($arr) && count($arr)){
			...
			if(count($this->arrFormErrors) != 0){
				...
				//si on est la c'est que l'on peut insert
				$query = 'INSERT INTO '.DB_PREFIX.'links (status, name, keyindex, path, language_id, extern, date_modified) VALUES("'.$arrValues['status'].'", "'.$arrValues['name'].'", "'.$arrValues['keyindex'].'", "'.$arrValues['path'].'", "'.$arrValues['language_id'].'", "'.$arrValues['extern'].'", NOW());';
				$this->reg->get('db')->query($query);
				//clean la cache
				$this->deleteCache();
				//retour
	...
	public function getLinksInfos($id){	
		$query = 'SELECT '.DB_PREFIX.'links.status AS "status", '.DB_PREFIX.'links.id AS "id", '.DB_PREFIX.'links.name AS "name", '.DB_PREFIX.'links.keyindex AS "keyindex", '.DB_PREFIX.'links.path AS "path", '.DB_PREFIX.'links.language_id AS "language_id", '.DB_PREFIX.'links.extern AS "extern" FROM '.DB_PREFIX.'links WHERE '.DB_PREFIX.'links.id = "'.$id.'" LIMIT 0,1;';
		$rs = $this->reg->get('db')->query($query);
		if($rs->num_rows){
			return $rs->rows[0];	
			}
	...
	public function setDirectLinksInfos($arrValues, $bRedirect = false){
		if($bRedirect){
			...
			if(isset($arrLink['path']) && ($arrLink['path'] != $arrValues['path'])){
				...
				$query = 'INSERT INTO '.DB_PREFIX.'links (keyindex, has_moved, access, language_id, name, path, status, hits, extern, ref_id, date_modified) VALUES("-", "1", "0", "'.$arrLink['language_id'].'", "-", "'.$arrLink['path'].'", "0", "0", "0", "'.$arrValues['id'].'" , NOW());';
				$this->reg->get('db')->query($query);
				}
			}
