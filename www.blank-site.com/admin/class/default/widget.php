...
class Widget {
	...
	public function getLinksForCheckboxByLang($langId){
		$query = 'SELECT '.DB_PREFIX.'links.id AS "id", '.DB_PREFIX.'links.path AS "path" FROM '.DB_PREFIX.'links WHERE '.DB_PREFIX.'links.extern = 0 AND '.DB_PREFIX.'links.language_id = '.intVal($langId).' ORDER BY '.DB_PREFIX.'links.path ASC;';
		$rs = $this->reg->get('db')->query($query);
		if($rs->num_rows){
			return $rs->rows;
			}
