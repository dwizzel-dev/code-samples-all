				...
				<div id="div-items-main" class="row-fluid">
					...
					if(is_array($arrOutput['content']['item']['details']['data-unserialize'])){
						...
						foreach($arrOutput['content']['item']['details']['data-unserialize'] as $k=>$v){
							...
							//opacity
							echo '<div class="control-group">';
							echo '<label class="control-label">'._T('opacity').'</label>';
							echo '<input type="text" value="';
							if($v['opacity'] == '' || intVal($v['opacity']) > 1){
								echo '1.0';
							}else{
								echo $v['opacity'];
								}
							echo '" class="form-control input-mini" placeholder="" id="modify-input-opacity-'.$cmptCarouselItems.'">';
							echo '</div>';
...
function insertItems(){
	...
	code += '</div>';
	//opacity
	code += '<div class="control-group"><label class="control-label"><?php echo _T('opacity'); ?></label>';
	code += '<input type="text" value="1.0" class="form-control input-mini" placeholder="" id="modify-input-opacity-' + gCmptItemBlock + '">';
	code += '</div>';	
	//background colors
	...
	if($('#modify-input-items_values')){
		...
		for(var o in arrPosition){
			...
			arrValues.push({
				...
				img:$('#modify-input-image-' + arrPosition[o]).val(),
				link:$('#modify-select-link-' + arrPosition[o]).val(),
				opacity:$('#modify-input-opacity-' + arrPosition[o]).val(),
				bgcolor:$('#modify-select-bgcolor-' + arrPosition[o]).val(),
				//seulement si on utilise de html editor	
