...
<div id="modal-modify-items" class="modal hide fade" tabindex="-1" role="dialog" aria-labelledby="" aria-hidden="true">
	...
	<div class="modal-body" id="modal-modify-items-body">
		...
		<form id="form-modify" method="post">
			...
			<hr>
			<label><?php echo _T('alias'); ?> :</label>
			<input type="text" name="alias" id="modal-modify-input-alias" class="w96">
...
//selected menu group
var gSelectedCategoryId = '<?php echo $arrOutput['filter']['categorie-id']; ?>';
var gSelectedLanguageId = '<?php echo $arrOutput['filter']['language-id']; ?>';
...
jQuery(document).ready(function($){
	...
	$('#buttcreate').click(function(e){
		...
		resetForm(true);
		//on met celui qui est selectionne dans les filtres
		$('#modal-modify-input-category_id').val(gSelectedCategoryId);
		$('#modal-modify-input-language_id').val(gSelectedLanguageId);
		});	
	//search box
...
function resetForm(bText){
	...
	$('#modal-modify-items-body').scrollTo(0, 500);	
	var arr = [['status', 'checkbox'], ['css_class', 'val'], ['name', 'val'], ['category_id', 'val'], ['language_id', 'val'], ['alias', 'val'], ['meta_title', 'val'], ['meta_description', 'val'], ['meta_keywords', 'val'], ['content', 'val'], ];
	for(var o in arr){
		if(bText){
			if(arr[o][1] == 'text'){
