...
@auth:	Dwizzel
@date:	00-00-0000
@info:	default news/news_category/xx/items-alias/ view
			...
			<form class="form-vertical" id="form-item">
				...
				<div class="control-group" style="margin-bottom:20px;">
					...
					if(SIMPLIFIED_SITE_PATH){
						?>
						<div><?php echo _T('path')?>: <a href="/<?php echo $arrOutput['content']['item']['details']['language'].'/'.$arrOutput['content']['item']['details']['path'].'/'.$arrOutput['content']['item']['details']['id'].'/'.$arrOutput['content']['item']['details']['alias'].'/'; ?>" target="_new">/<?php echo $arrOutput['content']['item']['details']['language'].'/'.$arrOutput['content']['item']['details']['path'].'/'.$arrOutput['content']['item']['details']['id'].'/'.$arrOutput['content']['item']['details']['alias'].'/'; ?></a></div>
					<?php	
					}else{
						$arrLangCodeByPrefix = $oGlob->get('languages_code_by_prefix');
						echo '<div>'._T('path').': <a href="'.PATH_WEB_SITE.'index.php?&lang='.$arrLangCodeByPrefix[$arrOutput['content']['item']['details']['language']].'&path='.$arrOutput['content']['item']['details']['path'].'/'.$arrOutput['content']['item']['details']['id'].'/'.$arrOutput['content']['item']['details']['alias'].'/" target="_new">'.PATH_WEB_SITE.'index.php?&lang='.$arrLangCodeByPrefix[$arrOutput['content']['item']['details']['language']].'&path='.$arrOutput['content']['item']['details']['path'].'/'.$arrOutput['content']['item']['details']['id'].'/'.$arrOutput['content']['item']['details']['alias'].'/</a></div>';
						}
					?>
					<div><?php echo _T('hits')?>: <?php echo $arrOutput['content']['item']['details']['hits']; ?></div>
					</fieldset>
				</div>
				...
				<div class="control-group">
					<label><?php echo _T('news category'); ?> :</label>
					<select name="category_id" id="modify-input-category_id">
					<?php
				...
				<div class="control-group">
					<label class="control-label"><?php echo _T('page preview')?></label>
					<div class="controls"><textarea rows="5" id="modify-input-preview" name="preview" class="span6"><?php echo $arrOutput['content']['item']['details']['preview']; ?></textarea></div>
				</div>
				<div class="control-group">
					<label class="control-label"><?php echo _T('page content')?></label>
...
jQuery(document).ready(function($){
	...
	$('#buttdelete').click(function(e){
		...
		$('#modal-win-content').html('<?php echo formatJavascript(_T('the news will be <b>deleted</b>, are you sure?')); ?>');
...
function addEditor(){
	var obj = {
		...
		content_css : [
			...
			'<?php echo PATH_CSS; ?>editor/news-item.css'
			],
		inline: false,
		height : 450,
		//preview plugin
		plugin_preview_type: 'news-item', //le type de widget pour le preview
		};	
	//editor
...
function deleteItem(){
	$.ajax({
		...
		data: {section:'news',service:'delete-item-infos',data:JSON.stringify($('#form-process').serializeArray())},
		success:function(data){
			...
			if(typeof(obj.msgerrors) !== "undefined" && obj.msgerrors){
				...
				window.location.href = '<?php echo $oGlob->getArray('links','news-listing').'page-'.$arrOutput['content']['news-listing-last-page-number'].'/'; ?>';
				}
			},
		error:function(){
			showButtLoadingTxt('#buttdelete', '<?php echo _T('delete'); ?>');
...
function disableItem(){
	$.ajax({
		type: 'POST',
		url: '<?php echo PATH_SERVICE; ?>',
		data: {section:'news',service:'disable-item-infos',data:JSON.stringify($('#form-process').serializeArray())},
		success:function(data){
			//parse data
			eval('var obj = ' + data + ';');
			if(typeof(obj.msgerrors) !== "undefined" && obj.msgerrors){
...
function enableItem(){
	$.ajax({
		type: 'POST',
		url: '<?php echo PATH_SERVICE; ?>',
		data: {section:'news',service:'enable-item-infos',data:JSON.stringify($('#form-process').serializeArray())},
		success:function(data){
			//parse data
			eval('var obj = ' + data + ';');
			if(typeof(obj.msgerrors) !== "undefined" && obj.msgerrors){
...
function saveDetails(){
	...
	$.ajax({
		type: 'POST',
		url: '<?php echo PATH_SERVICE; ?>',
		data: {section:'news',service:'set-item-infos',data:JSON.stringify($('#form-item').serializeArray())},
		success:function(data){
			//parse data
			//alert(data);
			eval('var obj = ' + data + ';');
...
function resetForm(){
	...
	$('#box-alert').hide();
	$('#box-alert-msg').text();
	var arr = [['meta_description', 'val'], ['meta_keywords', 'val'], ['meta_title', 'val'], ['name', 'val'], ['category_id', 'val'], ['preview', 'val'], ['css_class', 'val']];
	for(var o in arr){
