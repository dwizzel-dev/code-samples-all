...
@auth:	Dwizzel
@date:	00-00-0000
@info:	default widget-item module view
				...
				<div id="div-items-main" class="row-fluid">
					...
					if(is_array($arrOutput['content']['item']['details']['data-unserialize'])){
						...
						foreach($arrOutput['content']['item']['details']['data-unserialize'] as $k=>$v){
							...
							//le div de items
							//echo '<div id="div-main-items-'.$cmptCarouselItems.'" class="widget-block small">';
							...
							//content
							echo '<div class="control-group"><label class="control-label">'._T('content').'</label><div class="controls"><textarea rows="8" uid="'.$cmptCarouselItems.'" id="content-'.$cmptCarouselItems.'-html" class="w96 editable">'.$v['text'].'</textarea></div></div>';
...
function insertItems(){
	//increment le counter de bidule	
	gCmptItemBlock++;
	var arrIds = JSON.parse($('#modify-input-items_values').val());
	arrIds.push(gCmptItemBlock);
	$('#modify-input-items_values').val(JSON.stringify(arrIds));
	var code = '';
	//code += '<div id="div-main-items-' + gCmptItemBlock + '" class="widget-block small">';
	...
	//
	//on va chercher la position max aussi
...
function addEditor(strId){
	var obj = {
		...
		content_css : [
			...
			'<?php echo PATH_CSS; ?>editor/widget-module-item.css'
			],
		inline: false,
		height : 250,
		...
		init_instance_callback : function(editor){
			...
			this.settings.plugin_preview_title_input = 'modify-input-title-' + $('#' + editor.id).attr('uid');
			},
		//preview plugin
		plugin_preview_type: 'widget-module-item', //le type de widget pour le preview
	...
	if($('#modify-input-items_values')){
		...
		for(var o in arrPosition){
			...
			arrValues.push({
				...
				title:$('#modify-input-title-' + arrPosition[o]).val(),
