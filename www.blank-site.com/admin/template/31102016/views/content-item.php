	...
	<div class="row-fluid ">
		<div class="span12">
			...
			<form class="form-vertical" id="form-item">
				...
				<div class="control-group">
					<label class="control-label"><?php echo _T('alias')?></label>
					<div class="controls"><input type="text" id="modify-input-alias" name="alias" class="span6" value="<?php echo $arrOutput['content']['item']['details']['alias']; ?>" <?php echo ($arrOutput['content']['item']['details']['fixed'] == '1' ? 'disabled' : ''); ?> ></div>
				</div>
				<div class="control-group">
				...
				<?php 
					$strImage = $arrOutput['content']['item']['details']['bgimage'];									
					if($strImage == ''){
						$strImage = DEFAULT_NO_IMAGE;
						}
				?>		
				<div class="widget-block-div-image"><img class="img-polaroid" src="<?php echo PATH_WEB_MEDIA.$strImage; ?>"></div>
				<div class="control-group">
					<div class="controls input-append"><input name="bgimage" id="modify-input-image" type="text" value="<?php echo $arrOutput['content']['item']['details']['bgimage']; ?>" class="input-xlarge"><button type="button" class="btn" onclick="ImageSelector.select('modify-input-image');"><?php echo _T('bgimage'); ?></button></div>
				</div>
				<div class="control-group">
...
function addEditor(){
	var obj = {
		...
		content_css : [
			...
			'<?php echo PATH_CSS; ?>editor/content-item.css'
			],
		inline: false,
		height : 450,
		...
		init_instance_callback : function(editor){
			...
			this.settings.plugin_preview_image_input = 'modify-input-image';
			this.settings.plugin_preview_cssclass_input = 'modify-input-css_class';
			},
		//preview plugin
		plugin_preview_type: 'content-item', //le type de widget pour le preview
		};	
	//editor
...
function resetForm(){
	...
	$('#box-alert').hide();
	$('#box-alert-msg').text();
	var arr = [['meta_description', 'val'], ['meta_keywords', 'val'], ['meta_title', 'val'], ['alias', 'val'], ['name', 'val'], ['category_id', 'val'], ['language_id', 'val'], ['css_class', 'val']];
	for(var o in arr){
