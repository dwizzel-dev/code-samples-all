...
if($section == 'listing'){
	...
	$arrOutput['content']['widget']['columns'] = array('',_T('alias'),_T('category'),_T('lang'),_T('#'),_T('[nbsp]'));
	...
	$arrOutput['content']['language-dropdown'] = $oGlob->get('languages_for_dropbox');	
	//colors
	$arrOutput['content']['colors-dropdown'] = $oGlob->get('colors_for_dropbox');	
	//le data
	require_once(DIR_CLASS.'widget.php');
	$oWidget = new Widget($oReg);
	//dropdown
	...
	//le name du widget
	$arrOutput['content']['item']['h-title'] = $arrOutput['content']['item']['details']['name'].' ('.$oGlob->getArray('languages_prefix_by_id', $arrOutput['content']['item']['details']['language_id']).')';
	//clean
	unset($oWidget);
