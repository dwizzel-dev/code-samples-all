				...
				<div id="div-items-main" class="row-fluid">
					...
					if(is_array($arrOutput['content']['item']['details']['data-unserialize'])){
						foreach($arrOutput['content']['item']['details']['data-unserialize'] as $k=>$v){
							...
							if(!intVal($v['active'])){
								echo '<span class="input-group-btn"><button class="btn btn-secondary btn-danger" id="buttchangestatus-'.$cmptCarouselItems.'" activated="0">'._T('activer').'</button></span>';
							}else{
								echo '<span class="input-group-btn"><button class="btn btn-secondary btn-succes" id="buttchangestatus-'.$cmptCarouselItems.'" activated="1">'._T('desactiver').'</button></span>';
								}
							echo '<input type="text" class="form-control" placeholder="Product name">';
							echo '<span class="input-group-btn"><button class="btn btn-secondary" onclick="removeItems('.$cmptCarouselItems.')"><i class="icon-remove"></i></button></span>';
							echo '</div>';
							echo '</div>';
...
function insertItems(){
	...
	code += '<div class="btn-group pull-right"><button class="btn btn-danger" id="buttchangestatus-'+ gCmptItemBlock + '" activated="0"><?php echo _T('activer'); ?></button><button class="btn" onclick="removeItems('+ gCmptItemBlock + ')"><i class="icon-remove"></i></button></div>';	
	//images	
	...
	code += '</div>';
	//la position
	code += '<div class="control-group"><label class="control-label"><?php echo _T('position'); ?></label><div class="controls"><input type="text" id="modify-input-position-' + gCmptItemBlock + '" class="input-small" value="' + gCmptItemBlock + '"></div></div>';	
	//le title		
...
function saveCarousel(){	
	if($('#modify-input-items_values')){
		...
		for(var o in arrIds){
			...
			arrValues.push({
				active:$('#buttchangestatus-' + arrIds[o]).attr('activated'),
