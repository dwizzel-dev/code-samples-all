...
class ImageManager 
{
	...
	function _delFile($relative) 
	{
		...
		//on check pour le repertoire .responsive et .zoom
		$arrPathCompleteRelative = explode('/', $relative);
		$strFileName = array_pop($arrPathCompleteRelative);
		//on rajoute les deux path au path	
		$arrPathFileRelative = array(
			'zoom' => implode('/', $arrPathCompleteRelative).'/'.PATH_IMAGE_ZOOM.'/'.$strFileName,
			'responsive' =>	implode('/', $arrPathCompleteRelative).'/'.PATH_IMAGE_RESPONSIVE.'/'.$strFileName,
			);
		foreach($arrPathFileRelative as $k=>$v){
			$arrPathFileRelative[$k] = Files::makeFile($this->getBaseDir(), $v);
			}
		...
		if(Files::delFile($fullpath)){
			//on supprime les .zoom et .responsive
			foreach($arrPathFileRelative as $k=>$v){
				Files::delFile($v);
				}
			//le thumbnails de leditor
