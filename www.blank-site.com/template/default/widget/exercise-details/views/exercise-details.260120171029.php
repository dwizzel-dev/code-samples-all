...
class widgetExerciseDetailsView{
	...
	private function getDetails(&$arr, $css_class){	
		...
		//le title
		$strOutput = '<h1 itemprop="name">'.$arr['html'].'</h1>';
		//main div
		$strOutput .= '<div class="'.$css_class.'" id="'.$this->wname.'">';
		//div les photos
		$strOutput .= '<div class="ex-images" itemscope="" itemprop="primaryImageOfPage" itemtype="http://schema.org/ImageObject">';
		//images container
		$strOutput .= '<div>';
		//thumb0
		if($arr['thumb0'] != ''){
			$strOutput .= '<img itemprop="contentUrl" onerror="this.src=\''.PATH_IMAGE_DEFAULT.'\'" src="'.PATH_IMAGE_EXERCISE.$arr['thumb0'].'" alt="'.htmlSafeTag(ucfirst($arr['html'])).'" title="'.htmlSafeTag(ucfirst($arr['html'])).'">';
			}
		//thumb1
		if($arr['thumb1'] != ''){
			$strOutput .= '<img onerror="this.src=\''.PATH_IMAGE_DEFAULT.'\'" src="'.PATH_IMAGE_EXERCISE.$arr['thumb1'].'" alt="'.htmlSafeTag(ucfirst($arr['title'])).'" title="'.htmlSafeTag(ucfirst($arr['title'])).'">';
			}
		//close image container
		$strOutput .= '</div>';
		//title sous les images
		$strOutput .= '<p class="images-title"><span itemprop="name">'.$arr['title'].'</span> '.sprintf('(%s<span itemprop="author">%s</span>)', _T('picture from').' ', _T('physiotec hep software')).'</p>';
		//close le photos
		$strOutput .= '</div>';
		//la description
		$strOutput .= '<div class="ex-description" itemscope="" itemprop="mainContentOfPage" itemtype="https://health-lifesci.schema.org/WebPageElement"><h2 itemprop="name">'._T('home exercise description').'</h2><p itemprop="description">'.$this->getTaggedDescription($arr['keywords'], $arr['description']).'</p>';
		//modification date	
		$strOutput .= '<div class="ex-date-modified">'._T('last modification:').' <time itemprop="dateModified" datetime="'.$strDate.'">'.$strDate.'</time></div></div>';
		//
		return $strOutput;
		}
		...
		if(is_array($gLanguagesCode) && is_array($gLanguagesPrefix)){
			if(count($arr)){
				...
				foreach($arr as $k=>$v){
					...
					//
					$str .= '<li>';
					$str .= '<h4><a href="'.$strLinkSearch.'" title="'._T('search for exercise in').' '.htmlSafeTag($gLanguagesCode[$k]).'">'.$gLanguagesCode[$k].'</a></h4>';
