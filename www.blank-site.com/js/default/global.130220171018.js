...
function initCarousel(){
	...
	if(gIsMobile){
		...
		$('.carousel').each(function(){
			...
			var iCount = 0;
			var iInneredHeight = $(this).find('.innered').width();	
			...
			iDivider +=	0.1;
			var iBlockWidth = parseInt(iInneredHeight/iDivider);
	...
	$('.carousel').each(function(){
		...
		var iHeight = parseInt((parseInt($(this).find('.container .block .img').outerHeight()) - parseInt($(this).find('.cleft-arrow > IMG').outerHeight())))/2;
		...
		$(this).on('changePanel', function(obj, direction){
			...
			if(direction == 'left'){
				...
				iNewPanel++;
			}else if(direction == 'recalculate'){
				//si jamais sur un mobile on met juste la scroll bar
