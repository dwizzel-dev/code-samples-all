...
function JAutoComplete(args){
	...
	//no auto complete returned
	this.bNoAutoCompleteResult = true;
	...
	//si on enable le search automatic du premier LI
	//meme si ce nest pas un complete match dans le input box
	this.bEnablePartialMatch = false;			
		...
		*/
		...
		if(evnt.which != '13'){ 
			...
			if(str != '' && str.length >= this.minStrLen){
				...
				var bFetch = true;
				//quand le partial match suffit pour aller faire une recherche
				if(this.bEnablePartialMatch){
					//si pas de keyword IDs ont va aller chercher 
					//le premier de la iste (si il y a) par defaut
					if(this.getFocusedKwIds() == ''){
						//les ref du UL
						var strUlListingRef = '#' + this.baseDivId + ' > UL.listing';
						//le max de resultat
						var iMaxFocusId = parseInt($(strUlListingRef).attr('focus-id-max'));
						//on va voir si on a des result de LI
						if(iMaxFocusId === 0 || Number.isNaN(iMaxFocusId)){
							//pas de result alors on ne fait pas de recherche
							bFetch = false;
						}else{
							//on va cherche le keyword du premier LI comme si on cliquait dessus
							//on change le input box avec le contenu du LI
							this.setInputBoxText(params, $('#lisr1').text(), true);
							//on set les KWids focused
							this.setFocusedKwIds($('#lisr1').attr('keyword-ids'), $('#lisr1').attr('keyword-word'));
							//on set l'attribut
							$(strUlListingRef).attr('focus-id', 1);
							}
					}else{
	...
	*/
	this.fetchAutoCompleteDataRFS = function(obj, params, word, kwtype, cleanword, pid){
		...
		if(bContinue && params.refinput.is(':focus')){
			...
			//si a un resultat
			if(iNumRows > 0){
				this.bNoAutoCompleteResult = false;
				...
				if(typeof(obj[0].name) == 'string'){
					//IMPORTANT:
					// avant de setter le input-bg il va falloir voir si c'est un mot compose
					// et si il est complet sinon on fini seulement le dernier mot
				...
				*/
				...
				for(var o in obj){
					if(typeof(obj[o].name) == 'string' && (typeof(obj[o].id) == 'string' || typeof(obj[o].id) == 'number')){
						...
						//si son a un match complet c'Est a dire 
						//que les 3 mots sont trouve a l'interieur alors on garde celui-ci
						//en memoire, pas les autres suivant
						if(iCountMatch == arrNameWords.length && !bFoundCompleteMatch){
							this.debug('MATCH(' + arrNameWords.length + '):' + iCountMatch);
							bFoundCompleteMatch = true;
							//le premier LI	
							this.setFirstLiWord(obj[o].name);
							//on set les KWids focused
							this.setFocusedKwIds(obj[o].id, obj[o].name);
							//on va garder les infos de celui-ci
							//si tout les premier charaactere sont bons
							if(obj[o].name.indexOf(word) === 0){
								//le hint est OK
								bHint = true;
								//le premier LI	
								//this.setFirstLiWord(obj[o].name);
				...
				//si on ne trouve pas de match complet
