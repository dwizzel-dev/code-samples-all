	...
	public function fetchAutoCompleteData($data){
		...
		*/
		...
		if(isset($data['word']) && !empty($data['word'])){
			...
			$strPregMatch = $this->reg->get('db')->escape($this->wordPermutationForSQL($arrWordData));
			...
			if($rs && $rs->num_rows){
				...
				//loop
				foreach($rs->rows as $k=>$v){
					//ca c'est le kwtype que ,on a pour eviter 
					//le isset plus loin dans le code
					$iKeywordId = intVal($v['idKeyword']);
					//on push pour eviter les repetition et 
					//encode en meme temps
					if(!isset($arrResult[$iKeywordId])){
						//si existe pas
						$arrResult[$iKeywordId] = array(
							'word' => encodeString(mb_strtolower($v['keyword'], 'UTF-8')),
							'type' => $v['kwtype'],
							);
						}
					}
				...
				foreach($arrResult as $k=>$v){
					//check si existe
					if(!isset($arrByKwType[$v['type']])){
						$arrByKwType[$v['type']] = array();
						}
					//on push le data dedans selonle word 
					//on va additionner les keywordId
					//pour ne finir qu'avec un word et plusieurs keywordId
					if(!isset($arrByKwType[$v['type']][$v['word']])){
						//on le creer
						$arrByKwType[$v['type']][$v['word']] = $k;
					}else{
						//on rajoute le kw id
						$arrByKwType[$v['type']][$v['word']] .= ','.$k;
						}
