...
class Utility {
	...
	public function getClinicModule() {
		...
		$idClinic 	 = $clinic_data['idClinic'];
		$returnArr	 = array();
		$query = 'SELECT clinic_module.idModule, module.title ' .
			 'FROM module, clinic_module ' .
			 'WHERE module.idModule = clinic_module.idModule ' .
			 'AND clinic_module.idClinic="'.intval($idClinic).'";';
		$rs    = $this->reg->get("db")->query($query);
		if ($rs && $rs->num_rows) {
