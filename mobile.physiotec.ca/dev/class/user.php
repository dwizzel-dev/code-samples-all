...
class User {
	...
	public function pingService($data){
		...
		//so if session is not good service will bring the error to the client and disconnect him, 
	...
	public function getPrintParameters($data){
		...
		*/	
		...
		if(isset($data['programid']) && is_numeric($data['programid']) && $data['programid'] > 0 && isset($data['clientid']) && is_numeric($data['clientid']) && $data['clientid'] > 0){
			...
			$idLicence = intVal($arrLicenceData['idLicence']);
			//clean
			unset($arrLicenceData);
			//le idUser
			$idUser = $this->reg->get('sess')->get('idUser');
			//le defulat priont size
			$idPrint_size = 15;
			//le default type de print
			$defaultPrintType = 'picture';
			$arrPrintType = array(
				'picture'=>decodeString(translate('picture')),
				'drawing'=>decodeString(translate('drawing'))
				);
			//fetching the user print size
			$query = 'SELECT idPrint_size FROM user WHERE idUser= "'.intval($idUser).'" LIMIT 0,1;';
			$rs = $this->reg->get('db')->query($query);
			if($rs && $rs->num_rows){
			...
			$iPrintSizeProgramId = 0;
			$query = 'SELECT idPrint_size, img_type FROM program WHERE idProgram = "'.intval($data['programid']).'" AND idClient = "'.intval($data['clientid']).'" LIMIT 0,1;';
			$rs = $this->reg->get('db')->query($query);
			if($rs && $rs->num_rows){
				$row = $rs->row;
				$iPrintSizeProgramId = $row['idPrint_size'];
				$defaultPrintType = $row['img_type'];
				}
			//clean
			unset($rs);
			//
			if($iPrintSizeProgramId >= 15){
				// consider the print size
				$idPrint_size = $iPrintSizeProgramId;
				}
			if($idPrint_size < 15){
			...
			//get the parmas and titles of the prints available
			$arrPrintTitle = array();
			$arrPrintTemplate = array();
			$query = 'SELECT idPrint_size, title, template, exercise_per_page, orientation, with_tracking_sheet, font_size, image_size, bold, italic, with_summary FROM print_size WHERE (idLicence IS NULL OR idLicence = '.$idLicence.');';
			$rs = $this->reg->get('db')->query($query);
			if($rs && $rs->num_rows){
				foreach($rs->rows AS $k => $v){
					$arrPrintParams = array(
						0 => $v['exercise_per_page'],
						1 => $v['orientation'],
						2 => $v['image_size'],
						3 => $v['font_size'],
						4 => $v['bold'],
						5 => $v['italic'],
						6 => $v['with_tracking_sheet'],
						7 => $v['with_summary']
						);
					$arrPrintTemplate[$v['idPrint_size']] = implode('~', $arrPrintParams);;
					$arrPrintTitle[$v['idPrint_size']] = decodeString(translate($v['title']));
					}
				}
			//clean
			unset($rs);
			...
			$arr = array(
				...
				'type' => $arrPrintType,
				'size' => $arrPrintTitle,
				'template' => $arrPrintTemplate,
				'selected' => array(
					'format' => 'pdf',
					'type' => $defaultPrintType,
					'size' => $idPrint_size
					)
				);
			return $arr;
    ...
    public function autoLogin($data){
		if(isset($data['username']) && isset($data['password']) && $data['username'] != '' && $data['password'] != ''){
			$arrRtn = $this->doLogin($data, true);
			//minor check
			if(isset($arrRtn['sessid']) && $arrRtn['sessid'] !== 0 && $arrRtn['sessid'] !== false){
				//redirect
	...
	//-------------------------------------------------------------------------------------------------------------
    public function doLogin($data, $bAutologin = false){
		//base client login
		/*
		RECEIVER:
			data:{
